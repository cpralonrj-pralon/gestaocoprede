...
export const Header: React.FC<HeaderProps> = ({ toggleSidebar, activeModule, searchQuery, setSearchQuery }) => {
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            <div className="text-right hidden sm:block">
              <p className="text-xs font-bold text-slate-900 dark:text-white leading-none group-hover:text-primary transition-colors">Carlos Mendes</p>
              <p className="text-[10px] text-slate-500 font-medium mt-1">Diretor de Operações</p>
            </div>
            <div
              ...
              style={{ backgroundImage: `url('https://i.pravatar.cc/150?u=carlos')` }}
            />
          </div>
          ...
          {showProfileMenu && (
            <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-surface-dark border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl py-2 z-50 animate-in fade-in zoom-in duration-200">
              <MenuButton icon="person" label="Meu Perfil" />
              <MenuButton icon="settings" label="Configurações" />
              <MenuButton icon="shield_person" label="Privacidade" />
              <div className="h-px bg-slate-100 dark:bg-slate-800 my-1" />
              <MenuButton icon="logout" label="Sair do Sistema" color="text-red-500" />
            </div>
          )}
        </div>
      </div>
...
const MenuButton = ({ icon, label, color = "text-slate-700 dark:text-slate-200" }: any) => (
  <button className="w-full px-4 py-2 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-surface-highlight transition-colors text-left group">
